...
const createBooking = async (req, res) => {
  ...
  // Validate seat genders if provided
  if (seatGenders && Object.keys(seatGenders).length > 0) {
    for (const seat of seats) {
      if (!seatGenders[seat]) {
        return res.status(400).json({
          success: false,
          message: `Gender must be selected for seat ${seat}`,
        });
      }
      if (!['Male', 'Female'].includes(seatGenders[seat])) {
        return res.status(400).json({
          success: false,
          message: `Invalid gender for seat ${seat}. Must be Male or Female`,
        });
      }
    }
  } else if (!gender) {
    return res
      .status(400)
      .json({ success: false, message: "Gender information is required." });
  }
