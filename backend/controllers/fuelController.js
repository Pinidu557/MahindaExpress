...
export const listFuel = asyncHandler(async (req, res) => {
  try {
    ...
    console.error("Error in listFuel:", error);
    res.status(500).json({ message: "Failed to fetch fuel records", error: error.message });
  }
});
...
export const fuelReport = asyncHandler(async (req, res) => {
  try {
    ...
    console.error("Error in fuelReport:", error);
    res.status(500).json({ message: "Failed to generate fuel report", error: error.message });
  }
});
