...
export default function FuelPage() {
  ...
  const [form, setForm] = useState({ vehicleNumber: "", liters: "", costPerLiter: "", totalCost: "", odometer: "", date: "" });
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ vehicleNumber: "", liters: "", costPerLiter: "", totalCost: "", odometer: "", date: "" });
      setEditingId(null);
      await load();
      await loadReport();
    } catch (e) {
      toast.error(e?.response?.data?.message || (editingId ? "Failed to update fuel record" : "Failed to add fuel record"));
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <form onSubmit={onSubmit} className="card hover-lift animate-scaleIn max-w-2xl">
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="flex flex-col">
            <label htmlFor="liters" className="mb-1 font-medium">Liters</label>
            <input 
              id="liters"
              value={form.liters} 
          ...
          <div className="flex flex-col">
            <label htmlFor="costPerLiter" className="mb-1 font-medium">Cost per Liter</label>
            <input 
              id="costPerLiter"
          ...
          <div className="flex flex-col">
            <label htmlFor="totalCost" className="mb-1 font-medium">Total Cost</label>
            <input 
              id="totalCost"
          ...
          <div className="flex flex-col">
            <label htmlFor="odometer" className="mb-1 font-medium">Odometer</label>
            <input 
              id="odometer"
              value={form.odometer} 
        ...
        <div className="card hover-lift animate-scaleIn">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div className="bg-blue-50 p-3 rounded-lg">
              ...
              <div className="text-xl font-bold text-blue-900">{report.records}</div>
            </div>
            <div className="bg-green-50 p-3 rounded-lg">
              <div className="font-medium text-green-700">Total KM</div>
              <div className="text-xl font-bold text-green-900">{report.km?.toLocaleString() || 'N/A'}</div>
            </div>
            <div className="bg-yellow-50 p-3 rounded-lg">
              <div className="font-medium text-yellow-700">Total Liters</div>
              <div className="text-xl font-bold text-yellow-900">{report.liters?.toFixed(2) || 'N/A'}</div>
            </div>
            <div className="bg-purple-50 p-3 rounded-lg">
              <div className="font-medium text-purple-700">KM/L</div>
              <div className="text-xl font-bold text-purple-900">{report.kmPerL?.toFixed(2) || 'N/A'}</div>
            </div>
          </div>
        </div>
      )}
          ...
          <tbody>
            {records.length === 0 ? (
              ...
              <tr key={r._id} className="table-row animate-fadeIn" style={{animationDelay: `${index * 0.1}s`}}>
                ...
                <td className="p-2">{r.odometer?.toLocaleString() || 'N/A'}</td>
