    ...
    const fetchStaff = async () => {
      try {
        ...
        const payroll = response.data.map((member) => {
          const presentDays =
            member.attendance?.filter((a) => a.status === "Present").length ||
            0;
          ...
          const gross =
            baseSalary + allowances + bonus + reimbursements + otPay;
          const deductions =
            Math.max(0, (totalDays - presentDays) * (baseSalary / 30)) +
            salaryAdvance +
            loan; // Daily + advances/loan
    ...
    <div className="p-6 space-y-6 bg-slate-900 min-h-screen">
      <div className="mb-8">
        ...
        <p className="text-slate-400">
          Manage staff salaries and payroll calculations
        </p>
      </div>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-4">
            <label className="block text-sm font-medium text-slate-300 mb-0">
              Month:
            </label>
            <input
              type="month"
        ...
        {payrollData.length > 0 ? (
              ...
              <tbody className="divide-y divide-slate-700">
                {payrollData.map((member) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-200">
                        ${member.bonus.toLocaleString()}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-200">
                        ${member.reimbursements.toLocaleString()}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-200">
                        {member.otHours.toFixed(1)}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-200">
                        ${member.otPay.toLocaleString()}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-200">
                        ${member.salaryAdvance.toLocaleString()}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-200">
                        ${member.loan.toLocaleString()}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-slate-200">
                        ${member.gross.toLocaleString()}
                      </div>
                    </td>
                    <td className="px-6 py-4">
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-400">
              Add staff members to see payroll information
            </p>
          </div>
        )}
      </div>
