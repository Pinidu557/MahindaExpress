...
export default function MaintenancePage() {
  ...
  const [form, setForm] = useState({ vehicleNumber: "", serviceType: "", serviceDate: "", mechanicId: "", serviceCost: "", notes: "", mileageAtService: "", nextServiceDate: "", nextServiceMileage: "", status: "pending", partsUsed: [] });
  ...
  const loadMeta = async () => {
    try {
      ...
      toast.error("Failed to load maintenance reports/reminders");
    }
  };
  ...
  const onSubmit = async (e) => {
    ...
    if (serviceCostNum !== undefined && (!Number.isFinite(serviceCostNum) || serviceCostNum < 0)) {
      toast.error("Service cost must be a non-negative number");
      return;
    }
    const mileageAtServiceNum = numOrUndefined(form.mileageAtService);
    if (mileageAtServiceNum !== undefined && (!Number.isInteger(mileageAtServiceNum) || mileageAtServiceNum < 0)) {
    ...
    if (mileageAtServiceNum !== undefined && nextServiceMileageNum !== undefined && nextServiceMileageNum < mileageAtServiceNum) {
      toast.error("Next service mileage cannot be less than mileage at service");
      return;
    }
    ...
    const payload = {
      ...
      status: form.status,
      partsUsed: form.partsUsed.filter(p => p.part && p.qty).map(p => ({ part: p.part, qty: Number(p.qty) }))
    };
    try {
      ...
      setForm({ vehicleNumber: "", serviceType: "", serviceDate: "", mechanicId: "", serviceCost: "", notes: "", mileageAtService: "", nextServiceDate: "", nextServiceMileage: "", status: "pending", partsUsed: [] });
      setEditingId(null);
      await load();
      await loadMeta();
    } catch (e) {
      toast.error(e?.response?.data?.message || (editingId ? "Failed to update maintenance" : "Failed to add maintenance"));
    } finally {
  ...
  const onEdit = (r) => {
    ...
    setForm({
      ...
      partsUsed: (r.partsUsed || []).map(p => ({ part: p.part?._id || "", qty: p.qty ?? 1 }))
    });
  };
  ...
  const addPartRow = () => setForm(f => ({ ...f, partsUsed: [...f.partsUsed, { part: "", qty: "" }] }));
  const updatePartRow = (idx, key, value) => {
    const copy = [...form.partsUsed];
    copy[idx] = { ...copy[idx], [key]: value };
    setForm({ ...form, partsUsed: copy });
  };
  const removePartRow = (idx) => setForm({ ...form, partsUsed: form.partsUsed.filter((_, i) => i !== idx) });

  return (
    <div className="space-y-6 animate-fadeIn">
      <h1 className="text-2xl font-semibold animate-slideIn">Maintenance</h1>

      <form onSubmit={onSubmit} className="card hover-lift animate-scaleIn">
        <div className="grid grid-cols-3 gap-3">
          <div className="flex flex-col">
            <label htmlFor="vehicleNumber" className="mb-1 font-medium">Vehicle</label>
            <select 
              id="vehicleNumber"
            ...
            >
              ...
              {vehicles.map(v => (<option key={v._id} value={v.vehicleNumber}>{v.vehicleNumber} - {v.model}</option>))}
            </select>
          </div>
          <div className="flex flex-col">
            <label htmlFor="serviceType" className="mb-1 font-medium">Service Type</label>
            <input 
              id="serviceType"
          ...
          <div className="flex flex-col">
            <label htmlFor="serviceDate" className="mb-1 font-medium">Service Date</label>
            <input 
              id="serviceDate"
              type="date" 
          ...
          <div className="flex flex-col">
            <label htmlFor="mechanicId" className="mb-1 font-medium">Mechanic Name</label>
            <input 
              id="mechanicId"
          ...
          <div className="flex flex-col">
            <label htmlFor="serviceCost" className="mb-1 font-medium">Service Cost</label>
            <input 
              id="serviceCost"
          ...
          <div className="flex flex-col">
            <label htmlFor="mileageAtService" className="mb-1 font-medium">Mileage at Service</label>
            <input 
              id="mileageAtService"
          ...
          <div className="flex flex-col">
            <label htmlFor="nextServiceDate" className="mb-1 font-medium">Next Service Date</label>
            <input 
              id="nextServiceDate"
              type="date" 
          ...
          <div className="flex flex-col">
            <label htmlFor="nextServiceMileage" className="mb-1 font-medium">Next Service Mileage</label>
            <input 
        ...
        <div className="space-y-2">
          ...
          {form.partsUsed.map((row, idx) => (
            <div key={idx} className="grid grid-cols-3 gap-2 animate-fadeIn">
              <div className="flex flex-col">
                <label htmlFor={`part-${idx}`} className="mb-1 font-medium">Part</label>
                <select 
                  id={`part-${idx}`}
                  value={row.part} 
                ...
                >
                  ...
                  {parts.map(p => (<option key={p._id} value={p._id}>{p.name} ({p.stockQty})</option>))}
                </select>
              </div>
              <div className="flex flex-col">
                <label htmlFor={`qty-${idx}`} className="mb-1 font-medium">Quantity</label>
                <input 
                  id={`qty-${idx}`}
                  value={row.qty} 
        ...
        <div className="flex gap-2">
          <button 
            ...
            className={`${isSubmitting ? 'bg-gray-400 cursor-not-allowed' : 'btn-primary hover-glow'} flex items-center gap-2`}
          >
            {isSubmitting ? (
              <>
      ...
      {reminders && (
        <div className="card hover-lift animate-scaleIn">
          ...
          <div className="text-sm text-gray-700 mb-4 p-2 bg-gray-50 rounded">
            Window: {reminders.config.daysWindow} days / {reminders.config.kmWindow} km
          </div>
              ...
              <ul className="list-disc list-inside text-sm space-y-1">
                {reminders.dueByDate.map((r, index) => (
                  <li key={r._id} className="animate-slideIn" style={{animationDelay: `${index * 0.1}s`}}>
                    {r.vehicleNumber} - {r.serviceType} on {new Date(r.nextServiceDate).toLocaleDateString()}
                  </li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <ul className="list-disc list-inside text-sm space-y-1">
                {reminders.dueByMileage.map((r, index) => (
                  <li key={r._id} className="animate-slideIn" style={{animationDelay: `${index * 0.1}s`}}>
                    {r.vehicleNumber} - {r.serviceType} at {r.nextServiceMileage}km
                  </li>
                ))}
              </ul>
            </div>
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            {records.length === 0 ? (
              ...
              <tr key={r._id} className="table-row animate-fadeIn" style={{animationDelay: `${index * 0.1}s`}}>
                ...
                <td className="p-2">{r.serviceDate ? new Date(r.serviceDate).toLocaleDateString() : ""}</td>
      ...
      {report && (
        <div className="bg-white p-4 rounded shadow">
          ...
          <div className="text-sm">Total Records: {report.summary.records} • Total Cost: {report.summary.totalCost}</div>
          <div className="text-sm mt-2">By Service Type:</div>
          <ul className="list-disc list-inside text-sm">
            {report.byServiceType.map((x, i) => (
              <li key={i}>{x._id}: {x.count} records, {x.totalCost} cost</li>
            ))}
          </ul>
        </div>
      )}
