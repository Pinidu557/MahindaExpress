...
export default function StaffList({ refreshKey }) {
  ...
  async function handleAddOt(id) {
    ...
    try {
      ...
      setRows((r) =>
        r.map((x) =>
          x._id === id
            ? { ...x, otHours: updated?.otHours ?? (x.otHours || 0) + hours }
            : x
        )
      );
    } catch (e) {
  ...
  const filteredRows = rows.filter(
    (row) =>
      ...
      (row.assignedBus &&
        row.assignedBus.toLowerCase().includes(searchTerm.toLowerCase()))
  );
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm font-medium text-slate-200">
                        {Number(row.otHours || 0).toFixed(1)}
                      </span>
                    </td>
                    <td className="px-6 py-4">
